...
if(!["791610075130494987"].some(role => message.member.roles.cache.get(role)) && (!message.member.hasPermission("ADMINISTRATOR")))
return message.channel.send(new MessageEmbed().setDescription(`${message.author} Bu komutu kullanabilcek yetkiye sahip değilsin`).setColor('0x800d0d').setAuthor(message.member.displayName, message.author.avatarURL({ dynamic: true })).setTimestamp()).then(x => x.delete({timeout: 5000}));

const banlog = message.guild.channels.cache.find(c => c.id === '791459639023501362')//Ban log kanalı
...
if(kullanici.id === message.guild.OwnerID) return message.channel.send(new MessageEmbed().setDescription(`${message.author}, Sunucu sahibini yasaklayamazsın`).setAuthor(message.member.displayName, message.author.avatarURL({ dynamic: true })).setColor('0x800d0d').setTimestamp()).then(x => x.delete({timeout: 5000}));
...
message.channel.send(new MessageEmbed().setDescription(`${message.author} (\`${message.author.id}\`) tarafından ${kullanici} (\`${kullanici.id}\`) adlı kullanıcı **${sebep}** Sebebiyle Sunucudan Yasaklandı.`).setAuthor(message.member.displayName, message.author.avatarURL({dynamic: true})).setColor('0x348f36').setTimestamp())
