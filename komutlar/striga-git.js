...
return message.reply("Bu komutu kullanmadan önce bir ses kanalına giriş yapman gerekiyor");
}
...
if (!kullanıcı) return message.channel.send(`Bir kullanıcı belirtmen gerekiyor`);

let rol = message.mentions.roles.first();
let member = message.guild.member(kullanıcı);

if (!member.voice.channel) return message.channel.send(`Bu kullanıcı herhangi bir ses kanalında bulunmuyor`).then(m => m.delete(5000));
